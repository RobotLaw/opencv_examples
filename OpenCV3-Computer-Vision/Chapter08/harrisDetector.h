...
class HarrisDetector {
  ...
  public:
	  ...
	  void detect(const cv::Mat& image) {
		  ...
		  cv::minMaxLoc(cornerStrength,0,&maxStrength);//求最大角点响应值
			std::cout << "maxStrength:" << maxStrength << std::endl;
		  // local maxima detection
		  cv::Mat dilated;  // temporary image膨胀
		  cv::dilate(cornerStrength,dilated,cv::Mat());//将锚点的值替换为邻域内的最大值
			//std::cout << "dilated" << dilated << std::endl;
		  cv::compare(cornerStrength,dilated,localMax,cv::CMP_EQ);//localMax保留邻域内的局部最大值
			//结果图像类型是CV_8UC1，即单通道8位图，比较结果为真的地方值为255，否则为0；
			//cv::namedWindow("localMax");
			//cv::imshow("localMax",localMax);
	  ...
	  cv::Mat getCornerMap(double qualityLevel) {
		  ...
		  cv::threshold(cornerStrength,cornerTh,threshold,255,cv::THRESH_BINARY);//转化为cornerTh二值图像

		  // convert to 8-bit image
		  cornerTh.convertTo(cornerMap,CV_8U);
	
		  // non-maxima suppression
		  cv::bitwise_and(cornerMap,localMax,cornerMap);//只显示出现极大值的地方
