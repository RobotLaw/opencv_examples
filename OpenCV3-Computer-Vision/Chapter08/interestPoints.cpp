...
int main()
{
	...
	//cv::namedWindow("zhuanzhi");
	//cv::imshow("zhuanzhi",image);
	cv::flip(image, image, 0);//绕Ｘ轴翻转数组
	...
	//cv::imshow("cornerStrength", cornerStrength);
	//std::cout << "cornerStrength" << cornerStrength << std::endl;
   // threshold the corner strengths
	cv::Mat harrisCorners;	
	double threshold= 0.0001; 
	cv::threshold(cornerStrength, harrisCorners,
                  threshold,255,cv::THRESH_BINARY_INV);
	//harrisCorners = 255 - harrisCorners; 
    // Display the corners
	...
	cv::drawKeypoints(image,		// original image
		...
		-1,//cv::Scalar(255,255,255),	// color of the points
	...
	keypoints.clear();
	// FAST detector
	cv::Ptr<cv::FastFeatureDetector> ptrFAST = cv::FastFeatureDetector::create(40);//检测的点与中心点的阈值
	...
	int hstep(5), vstep(3); // a grid of 4 columns by 3 rows　网格数量，3行５列
	// hstep= vstep= 1; // try without grid
	int hsize(image.cols / hstep), vsize(image.rows / vstep);//每个网格的尺寸大小，hsize：水平尺寸，vsize：竖直尺寸
	int subtotal(total / (hstep*vstep)); // number of keypoints per grid，每个网格中要检测的特征点数量
	cv::Mat imageROI;
	std::vector<cv::KeyPoint> gridpoints;

	std::cout << "Grid of " << vstep << " by " << hstep << " each of size " << vsize << " by " << hsize << std::endl;

	// detection with low threshold，低阈值
	...
	for (int i = 0; i < vstep; i++)
		for (int j = 0; j < hstep; j++) {
			...
			if (gridpoints.size() > subtotal) {//网格中检测到的特征点数量大于给定值
			...
			for (auto it = gridpoints.begin(); it != itEnd; ++it) {
				it->pt += cv::Point2f(j*hsize, i*vsize); // convert to image coordinates，坐标转换一下
